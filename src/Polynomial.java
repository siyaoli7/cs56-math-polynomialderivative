...
public class Polynomial extends ArrayList<Integer> {
	...
	for(int i = this.getDegree(); i>=0; i--){
	    if(i==0) result += addD0(this.get(i),isFirst); 
	    else if(i==1) result += addD1(this.get(i),isFirst);
	    else result += addDH(this.get(i), i, isFirst);
	    isFirst = false;
	}
	return result;
    }

    private String addD0(int coef, boolean isFirst){
	String result = "";
	if(isFirst){
	    result += coef;
	}else{
	    if(coef<0) result = result + " - " + Math.abs(coef);
	    else if(coef == 0) result += "";
	    else result = result + " + " + coef;
	}
	return result;
    }
    private String addD1(int coef, boolean isFirst){
	String result = "";
	if(isFirst){
	    if(coef == 1) result += "x";
	    else if(coef == -1) result += "-x";
	    else result = result + coef + "x";
	}else{
	    if(coef == 1) result += " + x";
	    else if(coef == -1) result += " - x";
	    else if(coef == 0) result += "";
	    else result = addD0(coef, false) + "x";
	}
	return result;
    }
    private String addDH(int coef, int degree, boolean isFirst){
	String result = "";
	if(coef == 0) result = "";
	else result = result + addD1(coef, isFirst) + "^" + degree;
	return result;
    }
    ...
    */
	...
	for (int i = coeffsLowToHigh.length-1 ; i>=0; i--){
	    if(coeffsLowToHigh[i] != 0) return i;
       	}
	return 0; 
    }
    ...
    */
	...
	for(int i = 0; i < coeffsHighToLow.length ; i++){
	    if(coeffsHighToLow[i] != 0) return (coeffsHighToLow.length - i - 1);
	}
	return 0;
    }
    ...
    */
	...
	int [] newArr = new int [degreeCoeffsHighToLow(coeffsHighToLow)+1];
       	for(int i = coeffsHighToLow.length - 1; i >= coeffsHighToLow.length - 1 - degreeCoeffsHighToLow(coeffsHighToLow)  ; i--){
	    newArr[coeffsHighToLow.length-i-1] = coeffsHighToLow[i]; 
	}
	return newArr;
    }
    ...
    */
	...
	int [] newArr = new int [degreeCoeffsLowToHigh(coeffsLowToHigh)+1];
	for(int i = degreeCoeffsLowToHigh(coeffsLowToHigh); i>=0; i--){
	    newArr[newArr.length-i-1] = coeffsLowToHigh[i];
	}
	return newArr;
    }
    ...
    */
	...
	int newSize = Math.max(this.size(),p.size());
	int smallSize = Math.min(this.size(),p.size());
	int [] newArr = new int[newSize];
	for(int i=0; i<smallSize; i++){
	    newArr[i] = this.get(i)+p.get(i);
	}
	if(this.size() > smallSize){
	    for(int i = smallSize; i<newSize; i++){
		newArr[i] = this.get(i);
	    }
	}else if(p.size() > smallSize){
	    for(int i = smallSize; i<newSize; i++){
		newArr[i] = p.get(i);
	    }
	}
    ...
    */
	...
	Polynomial newP = new Polynomial (new int[]{0});
	int [] newA;
	Polynomial oneP;
	for(int i = 0; i<this.size();i++){
	    newA = new int [p.size()+i];
	    for(int k = 0; k < i; k++){
		newA[k] = 0;
	    }
	    for(int j = 0; j<p.size();j++){
		newA[j+i]=this.get(i)*p.get(j);
	    }
	    oneP = new Polynomial(Polynomial.highToLow(newA));
	    newP = newP.plus(oneP);
	}
	return newP; 
    ...
    */
	...
	Polynomial negOne = new Polynomial (new int[]{-1});
	return this.plus(p.times(negOne));
